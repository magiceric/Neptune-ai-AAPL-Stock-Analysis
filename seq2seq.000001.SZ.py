...
def rolling_predict(model=None, args=None, device=None, scaler=None):
    ...
    if args.feature == 'MS' or args.feature == 'S':
          ...
          pred_save_path = f"seq2seqCSVs/Interval_000001.SZ_{time.strftime('%Y%m%d')}.csv"
          df.to_csv(pred_save_path, index=False)
    else:
        df = pd.DataFrame(dict_of_lists)
        new_df = pd.concat((pre_data,df), axis=1)
        pred_save_path = f"seq2seqCSVs/Interval_000001.SZ_{time.strftime('%Y%m%d')}.csv"
        new_df.to_csv(pred_save_path, index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data_path', type=str, default='seq2seqCSVs\\000001.SZ.csv', help="你的数据数据地址")
    ...
    parser.add_argument('-roolling_data_path', type=str, default='seq2seqCSVs\\000001.SZ.test.csv', help="你滚动数据集的地址，此部分属于进阶功能")

    ...
    if request.method == 'POST':
        if stock_code and date_str and not error_message:
            try:
                ...
                # 根据指定的日期和天数格式化文件名
                filename = f"price_vol_{date_str.replace('-', '')}_{backtrack_days}.png"
                # 指定保存路径
                file_path = os.path.join(plot_directory, filename)
                # 保存fig_pricevol图像，如果文件已存在则直接覆盖
                if os.path.exists(file_path):
                    os.remove(file_path)
                fig_pricevol.write_image(file_path)
                        ...
                        rows.forEach(function(row) {{
                            if (row.innerText.includes(date_str)) {{
                                ...
                                if (nextRow2) {{
                                    ...
                                    var eighthColumnValue = nextRow2.cells[7].innerText;
                                    nextRow2.cells[7].style.fontWeight = 'bold'; // 使文本加粗
                                    var valueFloat = parseFloat(eighthColumnValue.replace(',', ''));
                                    if (valueFloat > 0) {{
                                        nextRow2.cells[7].style.color = 'red'; // 将文本颜色设置为红色表示正值
                                    }} else {{
                                        nextRow2.cells[7].style.color = 'green'; // 将文本颜色设置为绿色表示负值
                                    }}
